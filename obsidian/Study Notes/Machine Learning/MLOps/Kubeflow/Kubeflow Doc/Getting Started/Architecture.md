Kubeflow 프로젝트들과 각 프로젝트가 AI 라이프사이클의 어느 단계에 해당하는지를 소개

- [[#1. Kubeflow 생태계|1. Kubeflow 생태계]]
- [[#2. AI 라이프사이클 소개|2. AI 라이프사이클 소개]]
- [[#3. 프로덕션 및 개발 단계를 위한 AI 라이프사이클|3. 프로덕션 및 개발 단계를 위한 AI 라이프사이클]]
- [[#4. AI 라이프사이클 내의 Kubeflow 프로젝트|4. AI 라이프사이클 내의 Kubeflow 프로젝트]]
...
다음 다이어그램은 Kubeflow 생태계의 개요와 이것이 더 넓은 쿠버네티스 및 AI 환경과 어떻게 연관되는지를 보여줍니다. Kubeflow는 AI 플랫폼을 배포, 확장 및 관리하기 위한 시스템으로서 쿠버네티스 위에 구축되었습니다.

![](Pasted%20image%2020251204160009.png)

### 2. AI 라이프사이클 소개
(Introducing the AI Lifecycle)

AI 애플리케이션을 개발하고 배포할 때, AI 라이프사이클은 일반적으로 여러 단계로 구성됩니다. AI 시스템 개발은 반복적인(iterative) 과정입니다. 모델이 필요한 결과를 지속적으로 도출할 수 있도록 하려면, AI 라이프사이클의 다양한 단계에서 나오는 결과물을 평가하고 필요에 따라 모델과 파라미터에 변경 사항을 적용해야 합니다.

다음 다이어그램은 AI 라이프사이클 단계를 순서대로 보여줍니다:

![](Pasted%20image%2020251204160643.png)
...
5. **모델 서빙(Model Serving)** 단계에서는 <u>온라인 또는 배치 추론(inference)</u>을 위해 모델 아티팩트를 서빙합니다. 사용 사례에 따라 모델은 예측형(predictive) 또는 생성형(generative) AI 작업을 수행할 수 있습니다. 모델 서빙 단계에서는 온라인 피처 스토어를 사용하여 피처를 추출할 수도 있습니다. 모델 성능을 모니터링하고, 그 결과를 AI 라이프사이클의 이전 단계로 피드백합니다.
    

### 3. 프로덕션 및 개발 단계를 위한 AI 라이프사이클
(AI Lifecycle for Production and Development Phases)

AI 애플리케이션의 AI 라이프사이클은 개념적으로 <u>개발(development) 단계</u>와 <u>프로덕션(production) 단계</u>로 나눌 수 있으며, 이 다이어그램은 각 단계에 어떤 과정이 포함되는지 보여줍니다:

![](Pasted%20image%2020251204161836.png)

### 4. AI 라이프사이클 내의 Kubeflow 프로젝트
(Kubeflow Projects in the AI Lifecycle)

다음 다이어그램은 Kubeflow 프로젝트들이 AI 라이프사이클의 각 단계에 어떻게 부합하는지 보여줍니다:

![](Pasted%20image%2020251204161922.png)

각 Kubeflow 프로젝트에 대한 자세한 정보는 다음 항목을 참조하세요:
...
- **Kubeflow Pipelines**는 <u>AI 라이프사이클의 각 단계를 구축, 배포 및 관리</u>하는 데 사용할 수 있습니다.
...
![](Pasted%20image%2020251204161935.png)
