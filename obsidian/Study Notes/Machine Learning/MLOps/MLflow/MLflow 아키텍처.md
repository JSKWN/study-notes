...
- 로컬 호스트 및 DB 이용
- 추적 서버를 통한 원격 추적
![[obsidian/Attachments/Pasted image 20251203113252.png]]
...
| 설정    | 추가 설정 없음(기본값).                                                                                                              | 또는 환경 변수를 사용하여 추적 URI를 데이터베이스 URI(예: `sqlite:///mlflow.db`) 로 설정합니다 .`mlflow.set_tracking_uri``MLFLOW_TRACKING_URI`                                                                                                                     | [[https://mlflow.org/docs/latest/self-hosting/#docker-compose]] 설정을 참조하세요 . 또는 인기 클라우드 제공업체의 [관리형 MLflow 서비스를](https://www.databricks.com/product/managed-mlflow) 사용하여 유지 관리 오버헤드를 피할 수 있습니다. |
