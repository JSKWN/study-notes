...
---
- 조회: dict[key]
- 새로운 쌍 추가: dict[key] = value
- 키 or 값 요소 추출: (리스트 형변환 + 메서드), list(dict.keys()) or list(dict.items())
	- 키들을 가져올때는 .keys(), 쌍을 가져올때는 .items()임
	- .get(key) 메서드는 key에 대한 하나의 value를 조회한다는 것, 헷깔리지 말고
- 루프문 사용시: for key in dict.keys() 또는 for item in dict.items() (튜플형)
